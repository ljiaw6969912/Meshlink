...
- 创建服务器和加入已有网络分别显示对应面板；重新打开窗口时按已安装服务恢复模式、连接信息与已保存的邀请信息。
- 桌面控制台采用深蓝连接侧栏与浅色设备工作区，显示协调服务器、真实直连路径、设备地址和延迟；完整连接信息通过“连接详情”查看，设备管理与诊断位于顶部菜单。
- 创建或加入后由 Windows 自动启动服务保持连接，关闭窗口不影响后台运行；电脑重启后自动恢复。
- 节点上线后自动与同网在线设备协商直连；定时心跳和候选刷新在服务器恢复后自动重连，保留仍健康的设备直连。
- 创建服务器时同时启动本机设备节点，使用独立的虚拟地址与其他客户端双向直连，可通过设备列表访问服务器本机的远程桌面。
- Windows 多网卡环境按 UDP 收包时的本地地址和接口回复，避免服务器的握手响应误走另一张虚拟网卡。
- 支持读取、保存和管理节点配置。
- 支持运行状态记录、日志查看和基础诊断。
- 支持查看协调服务器状态、真实 P2P 路径、P2P UDP 监听地址、虚拟 IP、证书名称和 SHA256 证书指纹。
- 支持 RDP 可达性检查，并可调用系统远程桌面客户端连接目标主机。
- 设备列表同步状态时保留滚动位置和选中设备；远程桌面端口可编辑，默认 3389，并记住上次成功发起连接使用的端口。
...
“关于 Meshlink”中的更新地址默认使用当前服务器地址和连接端口，通过 `/updates` 获取最新发布，沿用本网络证书验证服务器身份。服务器运行新版后即可提供此接口。发布流程同步生成更新清单，版本下载 URL 映射固定的无配置压缩包；更新接口不提供配置、证书或目录浏览。

点击“立即更新并重启”会重新检查最新版本、下载并校验完整程序和随附脚本，再停止本安装目录的相关服务和进程后安装。原配置与身份保留，失败时回滚程序文件；成功后恢复原服务状态并重新打开桌面，核对运行版本及服务状态后提示“更新成功”。旧客户端仍可通过原有 1263 更新服务获取发布包；该兼容服务需要处于运行状态。
...
使用者应自行承担安装、配置、运行、部署和使用本项目所产生的全部风险。因使用、误用、无法使用本项目，或因配置错误、网络环境、系统权限、防火墙规则、证书管理、第三方组件、操作系统限制等原因导致的任何直接或间接损失，版权所有者不承担责任
